...
- Строку контекста из запроса пользователя передать **как есть, один раз**;
  из ответа cfsmcp2 копировать только **`effective_context`** / **`hit.context`**
  / **`contexts`** из JSON — **не** пересобирать CamelCase.
- `list_contexts` — когда контекст неизвестен или tool вернул `unknown`/`ambiguous`.
- **Не** повторять запрос по всем контекстам (fan-out).

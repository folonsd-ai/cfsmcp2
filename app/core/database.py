...
CREATE TABLE IF NOT EXISTS entities (
  ...
  embed_window_preset TEXT NOT NULL DEFAULT '',
...
_MIGRATIONS: list[tuple[str, str, str]] = [
    ...
    ("entities", "embed_window_preset", "TEXT NOT NULL DEFAULT ''"),
...
def migrate(conn: sqlite3.Connection) -> None:
    ...
    if _table_exists(conn, "objects") and _column_exists(conn, "objects", "guid"):
        conn.execute(
            "CREATE INDEX IF NOT EXISTS idx_objects_guid ON objects(entity_id, guid) "
            "WHERE guid != ''"
        )
    conn.execute(

...
TOOLS_RU: dict[str, dict] = {
    ...
    "find_code_references": {
        ...
        "description": (
            ...
            "На большом дампе обязателен parent_path из предыдущего хита; "
            "без него поиск по всему корпусу отклоняется. "
            "literal=true — предпочитать файлы на диске (свежий источник)."
        ),
        "parameters": {
            "context": _CTX_SINGLE,
            "identifier": "Идентификатор для поиска в телах методов BSL",
            "parent_path": "Префикс модуля/объекта из хита (на большом дампе обязателен)",
            "limit": "Макс. число хитов",
            "offset": "Смещение для постраничной выдачи",
            "literal": "Искать в файлах выгрузки; всегда с parent_path на большом дампе.",
        },
    },
    "get_skd": {

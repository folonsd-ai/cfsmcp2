...
mcp = FastMCP(
    ...
    instructions=(
        ...
        "UUID / xml identity → find_by_guid(context, guid) (dump; index or ConfigDumpInfo).\n"
...
def find_methods(
    ...
    literal: Annotated[
        ...
        Field(
            description=(
                "Mid-string LIKE on name/path/signature. Requires parent_path on large "
                "dumps; unscoped skips mid-string (exact/prefix only). Not for phrases."
            )
        ),
    ] = False,
) -> dict:
    ...
    literal=true does mid-string SQL only with parent_path; unscoped falls back
    to exact/prefix. Do not pass a natural-language phrase as literal.
    """
...
def get_method(
    ...
    If the dump file is newer than the last index stamp, body is read from disk and
    index_stale=true (search may still miss local edits until refresh).
    """
...
def find_code_references(
    context: Annotated[str, Field(description=_CTX_SINGLE_DESC)],
    identifier: Annotated[str, Field(description="Identifier to find in BSL method bodies")],
    parent_path: Annotated[
        str | None,
        Field(
            description=(
...
def find_by_guid(
    ...
    guid: Annotated[
        ...
        Field(
            description=(
                "Metadata UUID from XML uuid= / ConfigDumpInfo id= "
                "(braces and .N module suffix allowed)"
            )
        ),
    ],
    limit: Annotated[int, Field(description="Max hits", ge=1, le=50)] = 20,
) -> dict:
    """Resolve a dump metadata GUID to path/kind (index or ConfigDumpInfo.xml).

    Dump contexts only. Prefer get_object on the returned path.
    """

...
def bottleneck_hint(bottleneck: str, *, match_mode: str = "", expand: str = "") -> str:
    ...
    hints = {
        ...
        "open_coll_lock_wait": "wait on zvec store lock",
        "open_coll_load": "zvec.open() after lock",
        "parent_sql": "SQL load + lexical rank under parent_path",
        "parent_sql_probe": "lexical probe before zvec (metrics)",
        "fts": "SQLite FTS5 lexical search",
        "literal": "SQL substring match",
        "sql_catalog": "load method catalog for body search",
        "disk": "read dump .bsl files",
        "scan": "scan method bodies",
    }

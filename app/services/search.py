...
def get_method(context: str, path: str) -> dict:
    """Метод с телом; body отдаётся только при load_mode code/full."""
    conn = connect(settings.db_path)
    try:
        entity = resolve_context(context, conn)
        _require_bsl(entity, context)
        ...
        from_index = bsl_stores_body(m.get("load_mode")) and bool(body)
        truncated = False
        if from_index:
...
def find_code_references(
    ...
    *,
    ...
    try:
        ...
        if bsl_stores_body(mode):
            rows = conn.execute(
                """
                SELECT path, name, props_json
                FROM objects
            ...
            for r in rows:
                try:
                    ...
                    props = {}
                pp = str(props.get("parent_path") or "")
                if parent_filter and not (
                    pp == parent_filter or pp.startswith(parent_filter + ".")
                ):
                    continue
        ...
        return _find_code_references_in_dump(
            conn,
            entity,
            name,
            needle,
...
def _find_code_references_in_dump(
    ...
    *,
    ...
    rows = conn.execute(
        """
        SELECT path, name, props_json, source_rel
        FROM objects
        WHERE entity_id=? AND kind IN ('Procedure','Function')
        """,
        (int(entity["id"]),),
    ...
    for r in rows:
        try:
            ...
            props = {}
        pp = str(props.get("parent_path") or "")
        if parent_path and not (pp == parent_path or pp.startswith(parent_path + ".")):
            continue
        rel = str(r["source_rel"] or props.get("source_file") or "").strip()
        if not rel:
            continue
        try:
            decl = int(props.get("line") or 0)
        except (TypeError, ValueError):
            decl = 0

...
Версия документа: **1.5** · дата: **2026-08-18** · продукт **0.2.13**
...
│  GET /  ·  /static/                                         │
├─────────────────────────────────────────────────────────────┤
│  FastAPI (app/main.py)  version 0.2.13                      │
...
| `search_metadata` | FTS; prefix-expand длинных токенов; SQL LIKE fallback; ranking (kind/coverage/diversity при `tag:`; интенты, домены, Help) |
...
| `get_indexing_status` | **Все** сущности (не только ready) |
...
| `find_methods` | Exact substring → при &lt;3 hits **fuzzy** (zvec RRF) |
| `get_method` | Метод; `body` из индекса (`full`) или с диска (`signatures`/`signatures_only`, `body_source=file`) |
| `get_module_structure` | Методы, `#Область`, экспорты, номера строк |
| `find_code_references` | Подстрока в телах: индекс при `full`, иначе grep `.bsl` выгрузки по `source_rel` |

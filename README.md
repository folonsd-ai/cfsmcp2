...
| Поиск | `search_metadata` (`literal` = substring), `search_under`, `semantic_search`, `helpsearch` (справка), `get_required_fields`, `find_by_guid` (UUID → path) |
...
| BSL (`dump` + `bsl`) | `list_code_modules` (**`q` обязателен**, ≥2 символа), `list_methods`, `find_methods` (`literal`), `get_method` (`index_stale`), `get_module_structure`, `find_code_references` (`literal` → disk), `queue_method_reembed` |
| Формы | `search_forms`, `queue_managed_form_reembed` |
| СКД (`dump` + `dcs`) | `get_skd` |
| Статус | `get_indexing_status` (`index_stale` для dump) |
